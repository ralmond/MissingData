...
$\Sigma = {\bf A}^T{\bf A}$.
  ...
  - Note that ${\bf A}$ is the matrix that transforms a vector of
    independent uniform random normal valuses to the the correlated
    values: $ X = {\bf A}Z$.
...
* If $\Sigma$ is not positive definite, we will run into problems when
performing a regression.

Pair-wise deletion does not guarantee that the covariance matrix will
...
__Assume data are MCAR within strata

$$ \overline{y_{ps}} = \frac{1}{N} \sum_{j=1}^J N_j \overline
{y_{j,obs}} $$
...
$$\Var(\overline{y_{ps}}) = \frac{1}{N^2} \sum_{j=1}^J N_j^2
...
Instead of sampling from the predicted value, sample a residual as
well and add that to the prediction 

Improves the estimates of correlations, but it still overstates our
confidence in the sample (variances are too high and apparent power is
...
* Standard error of predict depends on what question you are asking:
  * How good is the prediction for the expected value (mean) of $Y$
   when  $X=  x_{*}$ ? (In SPSS Mean Prediction) 
  * How good is the prediction for an individual  $Y_i$  when
  * $X_i=x_{*}$?  (In SPSS Individual prediction)
...
The standard error for the mean can be estimated from the residual
variance  $s^2$.
...
* “What are likely values for the math score for James Goodfellow, who
* has a self-efficacy of 7.0?” 
...
* Sort the data set into groups based on complete variables (e.g.,
census tract)
...
* All items on the same scale (e.g., depression) are averaged and then
...
* In panel data, if somebody drops out, you can carry their last
   observation forward 
