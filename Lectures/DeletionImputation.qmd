...
-   For a normal distribution, a covariance matrix must be *positive definite*, that is, we must be able to find a matrix ${\bf A}$ , such that $\Sigma = {\bf A}^T{\bf A}$.

    -   Note that \$ X = {\bf A} Z + \mu \$, where $Z$ is independent unit normal vector.

    -   Can find ${\bf A}$ through the Cholesky decomposition, `chol(Sigma)`.
...
summarize(x,across(everything(),list(M=mean,S=sd),
                   .names = "{.col}.fn{.fn}"))
cor(Math)
...
pred <- predict(fitMP,data=Math10II)
Math10II$MathProb <- ifelse(is.na(Math10II$MathProb),
                            pred,Math10II$MathProb)
sum(is.na(Math10II$MathProb))
sum(is.na(Math10$MathProb))
...
```{r}
fit1 <- lm(MathProb ~IQ, data=X, na.action=na.omit)
...
s <- sum1$sigma
Xra <- diff(range(Math$MathProb,na.rm=TRUE))
Xbar <- mean(Math$MathProb,na.rm=TRUE)
Xmin <- min(Math$MathProb,na.rm=TRUE) -.05*Xra
Xmax <- max(Math$MathProb,na.rm=TRUE) + .05*Xra
X <- seq(Xmin,Xmax,Xra*1.1/100)

Y <- b[1]*X + b[2]
Yse<- 2*(b.se[2] + b.se[1]*abs(X-Xbar))


ggplot(Math) + aes(x=MathProb, y=IQ) + geom_point() +
  geom_abline(slope=b[1],intercept=b[2]) +
  geom_ribbon(aes(x=X,ymin=Y-2*Yse,ymax=Y+2*Yse))
